...
const MusicBoard = () => {
  return (
    <Stack component={'main'} width="100%" alignItems="center" justifyContent="center" display="flex" >
      <Box>
        Hello
      </Box>
              ...
              < div css={styles.board} >
                {
                  _.range(COLUMNS * ROWS).map((i) => (
                    <GridBackground columnIndex={i % COLUMNS} rowIndex={Math.floor(i / COLUMNS)} key={i} />
                  ))}
              </div>
...
const GridBackground: React.FC<GridBackgroundProps> = (props) => {
  ...
  return <span css={[styles.gridBackgroundCell, isGray && styles.grayCell, hasLeftBorder && styles.leftBorder, hasBoldLeftBorder && styles.boldBorder]} />
}

const DragDropLegend: React.FC = () => {
  return <Box css={styles.droppableLegend}>

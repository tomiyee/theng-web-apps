...
const FareShare: React.FC<FareShareProps> = () => {
  ...
  const onChange = (id: string, newValue: IndividualContribution) => {
    setItemizedBill(
      itemizedBill.map((individual) => (individual.id === id ? newValue : individual)),
    );
  };
  return (
    <Box component={'main'} width="100%" alignItems="center" justifyContent="center" display="flex">
      <Card>
        <CardContent sx={{ maxWidth: 600 }}>
          ...
          <Typography pb={1}>
            Split the bill based on how much each person bought. Those who bought less get to pay
            (proportionally) less! (e.g. If you are responsible for 50% of the cost, then you pay
            50% of the final total)
          </Typography>
          <Divider />
            ...
            <Grid2 size={8} display="flex" alignItems="center">
              <Button
                ...
                onClick={() =>
                  setItemizedBill([
                    ...itemizedBill,
                    { ...newIndividualContributor(itemizedBill.length + 1) },
                  ])
                }
              >
                Add a Person

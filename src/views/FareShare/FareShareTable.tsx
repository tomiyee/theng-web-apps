...
const FareShareTable: React.FC<FareShareTableProps> = (props) => {
  ...
  const billSubtotal = _.sum(
    itemizedBill.map((individual) =>
      _.sumBy(individual.itemizedContributions, (lineItem) => lineItem.cost),
    ),
  );
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ fontWeight: 'bold' }}>Personal Subtotal</TableCell>
            <TableCell sx={{ fontWeight: 'bold' }}>Percent of Subtotal</TableCell>
            <TableCell sx={{ fontWeight: 'bold' }}>Amount Owed</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {itemizedBill.map((individual) => {
            const personalSubtotal = _.sumBy(
              individual.itemizedContributions,
              (lineItem) => lineItem.cost,
            );
            return (
              <TableRow key={individual.id}>
                <TableCell>{individual.name}</TableCell>
                <TableCell>${personalSubtotal.toFixed(2)}</TableCell>
                <TableCell>
